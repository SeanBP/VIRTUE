...
public class EventLoader : MonoBehaviour
{
    ...
    private void ParseHeader(EventDataWrapper eventDataWrapper, bool chaseCompanion = true)
    {
        ...
        if (!inTour && chaseCompanion)
        {
            ...
            // project's scene -- no visible InputField UI for
            // time_before/time_after/speed. When the field exists, keep the
            // original behavior of writing its text (Update() parses that
            // back into timeBeforeCollision/speed each frame). When it
            // doesn't, apply the value directly to the runtime state
            // instead, the same way AnimateEventCoroutine already does for
            // tours (timeBeforeCollision = settings.time_before; speed =
            // settings.speed) -- the setting still takes effect, it just
            // has nothing to display it in.
    ...
    void Update()
    {
        if (loadingEvent == false)
        {
            ...
            // speedField/beforeField/afterField are null in this project (no
            // UI for them) -- skip re-reading from the field entirely
            // rather than letting float.Parse throw on a null .text access
            // every frame, which used to silently reset speed/
            // timeBeforeCollision/timeAfterCollision back to a hardcoded
            // default every frame, permanently overwriting whatever
            // ParseHeader had just applied directly from the event file's
            // header.
            try
            {
